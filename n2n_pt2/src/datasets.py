...
class RegDataset(AbstractDataset):    
    ...
    def __getitem__(self, index): ### this will also run when class is ran so this is where we rehook the system
        ...
        ## n2n_diff
        #gt=np.array(gt)
        #img2=np.array(img) #extracting gt and img2

        #gt=np.minimum(img2,gt) #taking min
        #gt=Image.fromarray(np.uint8(gt)) #restoring gt format
        #gt=gt.convert('RGB') #restoring gt format
