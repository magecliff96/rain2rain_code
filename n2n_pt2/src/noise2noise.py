...
class Noise2Noise(object):
    ...
    def _on_epoch_end(self, stats, train_loss, epoch, epoch_start, valid_loader):
        ...
        valid_loss, valid_time, valid_psnr = self.eval(valid_loader, epoch, self.p.nb_epochs) #added epoch
    ...
    def test(self, test_loader, show):
        ...
        for batch_idx, (source, target) in enumerate(test_loader):
            ...
            # Denoise
            denoised_img = self.model(source)[0].detach() #edited
            midlayer = self.model(source)[1].detach() #edited
            denoised_imgs.append(denoised_img)
            midlayers.append(midlayer) #edited

        # Squeeze tensors
        source_imgs = [t.squeeze(0) for t in source_imgs]
        denoised_imgs = [t.squeeze(0) for t in denoised_imgs]
        midlayers = [t.squeeze(0) for t in midlayers]
        ...
        for i in range(len(source_imgs)):
            ...
            create_montage(img_name, self.p.noise_type, save_path, source_imgs[i], denoised_imgs[i], clean_imgs[i], show) #edited
            montages((img_name+str(i)), save_path, source_imgs[i], midlayers[i][1], clean_imgs[i], show) #edited

    def eval(self, valid_loader, curr_epoch, total_epoch):
        ...
        for batch_idx, (source, target) in enumerate(valid_loader):
            ...
            # Denoise
            [source_denoised,placeholder] = self.model(source)

            # Update loss
            if self.p.loss=='l0':
                gamma=2*(1-((curr_epoch)/total_epoch)) ## edited
            else:
                gamma=1
            print('-'+str(gamma))

            loss = (self.loss(source_denoised, target)+0.00000001)**gamma# + self.loss2(source_denoised, target) + self.loss3(source_denoised, target))#edited
    ...
    def train(self, train_loader, valid_loader):
        ...
        for epoch in range(self.p.nb_epochs):
            ...
            for batch_idx, (source, target) in enumerate(train_loader):
                ...
                # Update loss
                if self.p.loss=='l0': #edited
                    gamma=2*(1-((epoch)/self.p.nb_epochs)) ## assume total ep always 10
                else:
                    gamma=1
