...
def parse_args():
    ...
    parser.add_argument('-tgt', '--train-gt-dir', help='training set path', default='./../data/train')### added ground truth for train
    parser.add_argument('-v', '--valid-dir', help='test set path', default='./../data/valid')
    parser.add_argument('-vgt', '--valid-gt-dir', help='test set path', default='./../data/valid') ### added validation for train
    ...
    parser.add_argument('-l', '--loss', help='loss function', choices=['l1', 'l2', 'hdr', 'dcp', 'l0'], default='l1', type=str)
    parser.add_argument('--cuda', help='use cuda', action='store_true')
    parser.add_argument('--plot-stats', help='plot stats after every epoch', action='store_true')

    # Corruption parameters
    parser.add_argument('-n', '--noise-type', help='noise type',
        choices=['gaussian', 'poisson', 'text', 'mc', 'none', 'rain'], default='gaussian', type=str) ## loads as noise_type
    parser.add_argument('-p', '--noise-param', help='noise parameter (e.g. std for gaussian)', default=50, type=float) ## loads as noise_param
...
if __name__ == '__main__':
    ...
    train_loader = load_dataset(params.train_dir,params.train_gt_dir, params.train_size, params, shuffled=True)
    valid_loader = load_dataset(params.valid_dir,params.valid_gt_dir, params.valid_size, params, shuffled=False)
